...
const UpdateScreen = () => {
  ...
  // Fetch the existing order details on component load
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/orders/${Number(id)}`)
        const { price, meal, side, drink, dessert } = response.data
        setPrice(price)
        setMeal(meal)
        setSide(side)
        setDrink(drink)
  ...
  // Submit updated order to backend
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const order: Omit<Order, "id"> = { price, meal, side, drink, dessert }
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}/orders/${Number(id)}`, order)
  ...
  return (
    <div className="p-10 min-h-screen">
      <h1 className="text-5xl font-bold mb-8">Update Order</h1>
